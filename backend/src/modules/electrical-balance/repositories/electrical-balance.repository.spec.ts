...
describe('ElectricalBalanceRepository', () => {
  ...
  describe('upsertByTimestamp', () => {
    ...
    it('should throw an error if timestamp is missing', async () => {
      ...
      await expect(repository.upsertByTimestamp(invalidData)).rejects.toThrow('Timestamp is required for upsert operation');
    });
    ...
    it('should throw an error if the update fails', async () => {
      ...
      await expect(repository.upsertByTimestamp(mockBalance)).rejects.toThrow(error);
    });
  });
  ...
  describe('findByDateRange', () => {
    ...
    it('should throw an error if start date is invalid', async () => {
      ...
      await expect(repository.findByDateRange(invalidDate, validDate)).rejects.toThrow('Invalid start date');
    });
    ...
    it('should throw an error if end date is invalid', async () => {
      ...
      await expect(repository.findByDateRange(validDate, invalidDate)).rejects.toThrow('Invalid end date');
    });
    ...
    it('should throw an error if query fails', async () => {
      ...
      await expect(repository.findByDateRange(startDate, endDate)).rejects.toThrow(error);
    });
  });
